...
class CryptoArbBot:
    ...
    def setup_handlers(self):
        ...
        handlers = [
            ...
            CallbackQueryHandler(self.button_handler, pattern="^(page|nav):"),
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [InlineKeyboardButton("🔴 Все отрицательные", callback_data="nav:negative:1")],
            [InlineKeyboardButton("🟢 Все положительные", callback_data="nav:positive:1")],
            ...
            [InlineKeyboardButton("💰 Ценовой арбитраж", callback_data="nav:price_arb")],
            [InlineKeyboardButton("📊 Статус бота", callback_data="nav:status")],
        ]
    ...
    async def show_funding_page(self, update: Update, context: ContextTypes.DEFAULT_TYPE, funding_type: str, page: int):
        ...
        quick_nav = []
        if total_pages > 5:
            for quick_page in [1, max(1, page-2), min(total_pages, page+2), total_pages]:
                if quick_page != page and quick_page not in quick_nav:
                    quick_nav.append(InlineKeyboardButton(str(quick_page), callback_data=f"page:{funding_type}:{quick_page}"))
            if quick_nav:
