...
class FundingRateBot:
    def __init__(self):
        ...
        self.exchanges = {
            ...
            # KCEX: требуется реальный публичный endpoint; сейчас заглушка
            "kcex": "https://api.kcex.com/futures/fundingRate",  # TODO: заменить на корректный URL KCEX
        }
    ...
    async def preload_intervals(self):
        ...
        async with aiohttp.ClientSession() as session:
            # Binance
            try:
                url_binance = "https://api.binance.com/fapi/v1/fundingInfo"
    ...
    def get_interval_hours(self, exchange: str, raw: Optional[Dict] = None) -> float:
        ...
        # Bitget: fundingRateInterval в часах (если поле есть в ответе)
    ...
    async def fetch_exchange_data(self, session: aiohttp.ClientSession, exchange: str, url: str) -> List[Dict]:
        ...
        try:
            # простой User-Agent, чтобы уменьшить шанс 418 от Binance/других
            headers = {"User-Agent": "FundingRateBot/1.0"}
    ...
    async def parse_exchange_data(self, exchange: str, data: dict) -> List[Dict]:
        ...
        allowed_intervals = [1, 2, 3, 4, 6, 8]  # Разрешенные интервалы (если нужно ограничивать)

        try:
            # --- BINANCE ---
            if exchange == "binance":
                if not isinstance(data, list):
                    logger.warning("Binance: неожиданный формат данных")
                    return funding_data
                ...
                for item in data:
                    ...
                    # Фильтрация по разрешенным интервалам (если список не пустой)
                    if allowed_intervals and interval_hours not in allowed_intervals:
                        continue
            ...
            # --- OKX ---
                ...
                # Для каждого инструмента отдельно запрашиваем funding-rate
                async with aiohttp.ClientSession() as session:
                    headers = {"User-Agent": "FundingRateBot/1.0"}
                    for inst in instruments:
                        ...
                        try:
                            async with session.get(fr_url, timeout=10, headers=headers) as resp:
                                if resp.status != 200:
            ...
            # --- HTX ---
                ...
                # ответ вида: {"status":"ok","data":[{...,"funding_rate":"0.0001",...}], "ts":...}
                if data.get("status") == "ok":
                    for item in data.get("data", []):
                        symbol = item.get("contract_code") or item.get("symbol")
                        if not symbol:
                            continue

                        fr_raw = item.get("funding_rate") or item.get("fundingRate")
                        if fr_raw is None:
                            continue
                        try:
                            funding_rate = float(fr_raw) * 100.0
            ...
            # --- MEXC ---
                ...
                # контрактный API MEXC: {"success":true,"code":0,"data":[{...,"symbol":"BTC_USDT","fundingRate":"0.0001",...}, ...]}
                if data.get("success") in (True, "true") and data.get("code") in (0, "0"):
                    for item in data.get("data", []):
                        symbol = item.get("symbol", "")
                        if not symbol.endswith("_USDT"):
                            continue

                        fr_raw = item.get("fundingRate") or item.get("funding_rate")
                        if fr_raw is None:
                            continue
                        try:
                            funding_rate = float(fr_raw) * 100.0
                        ...
                        funding_data.append(
                            self.enrich_with_yield(exchange, norm_symbol, funding_rate, interval_hours)
                        )
                else:
                    logger.warning(f"MEXC: неожиданный формат ответа или код: {data}")

            # --- LBANK ---
            elif exchange == "lbank":
                # ожидаемый формат: {"result":"true","data":[{"symbol":"BTCUSDT","fundingRate":"0.0001",...},...]}
                ok_values = {True, "true", "True", "SUCCESS", "success"}
                if data.get("result") in ok_values or data.get("success") in ok_values:
                    items = data.get("data", [])
                    if isinstance(items, dict):
                        items = items.get("fundingRate", []) or items.get("list", [])
                    for item in items:
                        ...
                        funding_data.append(
                            self.enrich_with_yield(exchange, str(symbol).upper(), funding_rate, interval_hours)
                        )
                else:
                    logger.warning(f"LBank: неожиданный формат ответа: {data}")

            # --- GATE ---
            elif exchange == "gate":
                # /api/v4/futures/usdt/contracts -> список контрактов
                if isinstance(data, list):
                    for item in data:
                        # symbol / name вида BTC_USDT
                        symbol = item.get("name") or item.get("symbol")
                        if not symbol or not str(symbol).endswith("_USDT"):
                            continue
                        ...
                        norm_symbol = str(symbol).replace("_", "")
                        funding_data.append(
                            self.enrich_with_yield(exchange, norm_symbol, funding_rate, interval_hours)
                        )
                else:
                    logger.warning("Gate: неожиданный формат ответа (ожидался список контрактов)")

            # --- BINGX ---
            elif exchange == "bingx":
                # /openApi/swap/v2/quote/fundingRate -> {"code":0,"msg":"success","data":[{"symbol":"BTC-USDT","fundingRate":"0.0001",...},...]}
                ...
                for item in items:
                    ...
                    if "USDT" not in symbol_str:
                        continue

                    fr_raw = item.get("fundingRate") or item.get("lastFundingRate")
                    if fr_raw is None:
                        continue
                    try:
                        funding_rate = float(fr_raw) * 100.0
                    ...
                    norm_symbol = symbol_str.replace("-", "").replace("_", "")
                    funding_data.append(
                        self.enrich_with_yield(exchange, norm_symbol, funding_rate, interval_hours)
                    )
            ...
            # --- KCEX (заглушка) ---
                ...
                # ❗ Публичной документации по KCEX в открытом доступе нет.
                # Здесь только заглушка, чтобы не ломать код.
                logger.info("KCEX: парсер пока не реализован, требуется реальный API endpoint")
    ...
    def format_funding_message(self, data: List[Dict], start_idx: int = 0, limit: int = 20) -> str:
        ...
        total_pages = (len(data) - 1) // limit + 1
        current_page = start_idx // limit + 1
        message = f"Страница {current_page}/{total_pages}\n\n"
    ...
    def format_arbitrage_message(self, opportunities: List[Dict], start_idx: int = 0, limit: int = 10) -> str:
        """Формирование сообщения для арбитража с пагинацией"""
        if not opportunities:
            return "Арбитражные возможности не найдены"

        end_idx = min(start_idx + limit, len(opportunities))
        page_data = opportunities[start_idx:end_idx]

        total_pages = (len(opportunities) - 1) // limit + 1
        current_page = start_idx // limit + 1
...
async def handle_pagination(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    page_size = session_data.get("page_size", 20)
    ...
    if action == "prev":
        ...
        new_start = min(start + page_size, max(0, (total - 1) // page_size * page_size))
    elif action == "first":
        new_start = 0
    elif action == "last":
        new_start = max(0, (total - 1) // page_size * page_size)
    else:
        # неизвестное действие, оставляем текущую страницу
        new_start = start
    ...
    if data_type in ("negative", "positive"):
        message_text = bot.format_funding_message(all_data, new_start, page_size)
    elif data_type == "arbitrage":
        message_text = bot.format_arbitrage_message(all_data, new_start, page_size)
    else:
        message_text = "Неизвестный тип данных."

    # Создаем кнопки пагинации с быстрым перемещением
    keyboard_rows = []
    nav_row = []

    if new_start > 0:
        nav_row.append(InlineKeyboardButton("⏮ В начало", callback_data="first_"))
        nav_row.append(InlineKeyboardButton("⬅️ Назад", callback_data="prev_"))

    if new_start + page_size < total:
        nav_row.append(InlineKeyboardButton("Вперед ➡️", callback_data="next_"))
        nav_row.append(InlineKeyboardButton("В конец ⏭", callback_data="last_"))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if message_text == "📉 Все фандинги (отрицательные)":
            ...
            message_page = bot.format_funding_message(sorted_data, 0, 20)
            ...
            if len(sorted_data) > 20:
                keyboard_rows.append(
                    [
                        InlineKeyboardButton("Вперед ➡️", callback_data="next_"),
                        InlineKeyboardButton("В конец ⏭", callback_data="last_"),
                    ]
                )

            reply_markup = InlineKeyboardMarkup(keyboard_rows) if keyboard_rows else None

            await update.message.reply_text(message_page, reply_markup=reply_markup)
            ...
            msg_neg = bot.format_funding_message(top_negative, 0, len(top_negative))
            msg_pos = bot.format_funding_message(top_positive, 0, len(top_positive))
            ...
            if not opportunities:
                await update.message.reply_text("Арбитражные возможности не найдены")
                return
            ...
            message_page = bot.format_arbitrage_message(opportunities, 0, 10)

            # Кнопки пагинации для арбитража
            keyboard_rows = []
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_pagination, pattern="^(prev|next|first|last)_"))
